...
class SubTreeDataLoaderNodeQuerySpec extends Specification  {
  ...
  def writeBinaryTreeInDb(documentSetId: Long, depth : Int)(implicit connection: Connection) : List[Long] = {
    val id = insertNode(documentSetId, None, "root")

    id :: writeSubTreeInDb(documentSetId, id, depth - 1) 
  }

  def writeSubTreeInDb(documentSetId: Long, root: Long, depth: Int)(implicit connection : Connection) : List[Long] = depth match {
    case 0 => Nil
    case _ => {
