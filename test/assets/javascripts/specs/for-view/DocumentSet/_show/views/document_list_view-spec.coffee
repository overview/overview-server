require [
  ...
  'for-view/DocumentSet/_show/views/document_list_view'
], ($, i18n, observable, DocumentListView) ->
  ...
  describe 'views/document_list_view', ->
    beforeEach ->
      i18n.reset_messages({
        'views.DocumentSet.show.document_list.document.empty_description': 'empty'
        'views.DocumentSet.show.document_list.loading': 'loading'
        'views.DocumentSet.show.document_list.title.num_documents': '{0} documents'
        'views.DocumentSet.show.document_list.title.node': 'node {0}'
        'views.DocumentSet.show.document_list.title.tag': 'tag {0}'
        'views.DocumentSet.show.document_list.title.n_nodes_and_n_tags': '{0} nodes {1} tags'
        'views.DocumentSet.show.document_list.title_html': '<b>{0}</b> in <b>{1}</b>'
        'views.DocumentSet.show.document_list.title.edit': 'edit'
      })
    ...
    describe 'DocumentListView', ->
      ...
      describe 'starting with an incomplete list', ->
        ...
        it 'should show the number of documents', ->
          view = create_view()
          expect($('h4', view.div).html()).toEqual('<b>10 documents</b> in <b>0 nodes 0 tags</b>')
      ...
      describe 'starting with just placeholder data', ->
        ...
        it 'should show a loading message instead of the number of documents', ->
          ...
          expect(num_documents_text).toEqual('loading')
