...
class showSpec extends Specification  {
  ...
  "DocumentSet view generated Json" should {
	...
	.23, "running")
      override val documentSet = DocumentSet(1, "a title", "a query", Some(20), Some(job))
    }

    trait NotStartedDocumentSetContext extends DocumentSetContext {
      class FakeNotStartedJob extends
	DocumentSetCreationJob(1, Some("name"), Some("password"), NotStarted, 23, "running") {
	  override val jobsAheadInQueue = 5l
	}

      val job = new FakeNotStartedJob
      override val documentSet = DocumentSet(1, "a title", "a query", Some(20), Some(job))
    }
