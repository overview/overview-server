...
class XlsxSpreadsheetContentSpec extends Specification with AwaitMethod {
  trait SpreadsheetScope extends Scope {
    ...
    lazy val text: String = {
      ...
      await(XlsxSpreadsheetContent(Rows(headers, rows), write _).map(_ => outputBuffer.toString))
    }
  }
