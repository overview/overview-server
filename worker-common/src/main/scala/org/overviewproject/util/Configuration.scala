...
class SearchIndexConfiguration(val configuration: Config) extends DefaultConfigurable {
  val prefix: String = "search_index"

  private val ConfigFile = "config_file"

  val defaultValues: Map[String, Any] = Map(
    (ConfigFile -> "elasticsearch.yml")    
  )
   
  val configFile: String = getString(ConfigFile)
}
