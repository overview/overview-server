  ...
  trait DbScope extends Scope with AwaitMethod with HasBlockingDatabase {
    ...
    private def clearDb: Unit = {
      ...
      blockingDatabase.runUnit(sqlu"""
        ...
        q14 AS (DELETE FROM document_set_file2),
        q15 AS (DELETE FROM file2),
        q16 AS (DELETE FROM document),
        q17 AS (DELETE FROM uploaded_file),
        q18 AS (DELETE FROM upload),
        q19 AS (DELETE FROM "view"),
        q20 AS (DELETE FROM document_set_user),
        q21 AS (DELETE FROM document_processing_error),
        q22 AS (DELETE FROM document_cloud_import_id_list),
        q23 AS (DELETE FROM document_cloud_import),
        q24 AS (DELETE FROM api_token),
        q25 AS (DELETE FROM plugin),
        q26 AS (DELETE FROM "session"),
        q27 AS (DELETE FROM "user"),
        q28 AS (DELETE FROM csv_import),
        q29 AS (DELETE FROM clone_job),
        q30 AS (DELETE FROM document_set_reindex_job),
        q31 AS (DELETE FROM document_id_list)
