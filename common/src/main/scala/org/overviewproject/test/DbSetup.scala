...
object DbSetup {
  ...
  def insertCsvImportDocumentSet(uploadedFileId: Long)(implicit c: Connection): Long = {
    SQL("""
      INSERT INTO document_set (type, public, title, uploaded_file_id, created_at)
      VALUES ('CsvImportDocumentSet', 'false', {title}, {uploadedFileId}, TIMESTAMP '1970-01-01 00:00:00')
      """).on(
      'title -> "Csv Import",
  ...
  def insertDocumentSet(query: String)(implicit c: Connection): Long = {
    SQL("""
      ...
      VALUES ('DocumentCloudDocumentSet'::document_set_type, 'false', {title}, {query}, TIMESTAMP '1970-01-01 00:00:00')
      """).on(
