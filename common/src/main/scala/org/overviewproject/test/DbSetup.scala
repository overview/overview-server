...
object DbSetup {
  ...
  def insertCsvImportDocumentSet(uploadedFileId: Long)(implicit c: Connection): Long = {
    SQL("""
      ...
      VALUES ('CsvImportDocumentSet', 'false', {title}, {uploadedFileId}, TIMESTAMP '1970-01-01 00:00:00', 100, 0, 0)
      """).on(
      'title -> "Csv Import",
  ...
  def insertDocumentSet(query: String)(implicit c: Connection): Long = {
    SQL("""
      INSERT INTO document_set (type, public, title, query, created_at, document_count, document_processing_error_count, import_overflow_count)
      VALUES ('DocumentCloudDocumentSet'::document_set_type, 'false', {title}, {query}, TIMESTAMP '1970-01-01 00:00:00', 100, 0, 0)
      """).on(
