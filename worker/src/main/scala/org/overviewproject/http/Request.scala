...
case class PrivateRequest(url: String, credentials: Credentials) extends Request {
  override def execute(client: Client, responseHandler: AsyncCompletionHandler[Unit]): Unit = client.submitWithAuthentication(url, credentials, responseHandler)
}