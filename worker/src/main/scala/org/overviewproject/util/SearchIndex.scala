...
class SearchIndex2 {
  ...
  private val node: Node = nodeBuilder.settings(ImmutableSettings.settingsBuilder.loadFromClasspath(ConfigFile)).node 
  private def client = node.client
  private def admin = client.admin.indices
  ...
  private def indexExists: Boolean = {
    ...
    Logger.info(s"Exists? ${r.isExists()}")
    r.isExists
  }
