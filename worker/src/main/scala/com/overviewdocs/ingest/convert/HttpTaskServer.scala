...
import akka.stream.{ActorMaterializer,OverflowStrategy,Materializer,SourceShape}
import akka.stream.scaladsl.{GraphDSL,Keep,MergePreferred,Sink,Source}
...
class HttpTaskServer(
  ...
  implicit val mat: Materializer = ActorMaterializer.create(actorRefFactory)
  ...
  private val workerTaskPoolRef = actorRefFactory.actorOf(WorkerTaskPool.props(maxNWorkers, workerIdleTimeout, timeoutActorRef))
  ...
  private val retryingTaskSource: Source[Task, akka.NotUsed] = {
    Source.fromGraph(GraphDSL.create() { implicit builder =>
      ...
      val normal = builder.add(taskSource)
  ...
  private val taskProviderRef = actorRefFactory.actorOf(TaskProvider.props(httpCreateIdleTimeout))
  retryingTaskSource
  ...
  private def drainEntity(entity: HttpEntity): Future[akka.Done] = {
  ...
  /** Turns an input into a fragment.
    ...
    */
  private def partToFragment(name: String, entity: HttpEntity): Future[StepOutputFragment] = {
