...
import com.overviewdocs.query.{Field,FuzzyTermQuery,NotQuery,PhraseQuery,PrefixQuery,Query,RegexQuery}
  ...
  "DocumentSetLuceneIndex" should {
    "#addDocuments" should {
      "index metadata fields and maintain field count" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L)))
        index.nMetadataFields must beEqualTo(0)
        index.addDocuments(Vector(buildDocument(2L).copy(metadataJson=Json.obj("foo" -> "bar"))))
        index.nMetadataFields must beEqualTo(1)
        index.addDocuments(Vector(buildDocument(3L).copy(metadataJson=Json.obj("foo" -> "bar"))))
        index.nMetadataFields must beEqualTo(1)
        index.addDocuments(Vector(buildDocument(4L).copy(metadataJson=Json.obj("bar" -> "baz"))))
        index.nMetadataFields must beEqualTo(2)
        index.addDocuments(Vector(buildDocument(5L).copy(metadataJson=Json.obj("foo" -> "baz", "bar" -> "baz"))))
        index.nMetadataFields must beEqualTo(2)
      ...
      "silently refuse to index >maxNMetadataFields fields" in new BaseScope {
        ...
        index.addDocuments(Vector(buildDocument(1L).copy(metadataJson=Json.obj("foo" -> "bar", "bar" -> "baz"))))
        index.addDocuments(Vector(buildDocument(2L).copy(metadataJson=Json.obj("baz" -> "foo"))))
        index.nMetadataFields must beEqualTo(2)
        index.metadataFields.keys must beEqualTo(Set("foo", "bar"))
      }
    }
    ...
    "#searchForIds" should {
      "find a document" in new BaseScope {
        index.addDocuments(Vector(buildDocument(12345L), buildDocument(23456L)))
      ...
      "not find documents we have since updated" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L).copy(title="bad title")))
        index.updateDocuments(Vector(buildDocument(1L).copy(title="good title")))
      ...
      "find a term indexed from NFC and searched from NFD" in new BaseScope {
        ...
        index.addDocuments(Vector(document))
      ...
      "find multiple documents" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L), buildDocument(2L), buildDocument(3L)))
        search(PhraseQuery(Field.All, "bar")) must beEqualTo("1,2,3")
      }

      "handle FuzzyTermQuery" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L), buildDocument(2L), buildDocument(99L)))
      ...
      "avoid Lucene maxEdits error with FuzzyTermQuery" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L), buildDocument(2L)))
      ...
      "handle pdfNotes query" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L), buildDocument(2L).copy(pdfNotes=PdfNoteCollection(Array(
      ...
      "handle metadata query" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L), buildDocument(2L).copy(metadataJson=Json.obj("foo" -> "bar"))))
      ...
      "pass-through regex query" in new BaseScope {
        // We process `text:/bar/` in the regex phase, not the Lucene phase
        index.addDocuments(Vector(
          buildDocument(1L).copy(title="foo/bar/baz.txt"),
          buildDocument(2L).copy(title="foo/baz.txt")
        ))
      ...
      "pass-through not-regex query" in new BaseScope {
        // We process `NOT text:/bar/` in the regex phase, not the Lucene phase
        index.addDocuments(Vector(
          buildDocument(1L).copy(title="foo/bar/baz.txt"),
          buildDocument(2L).copy(title="foo/baz.txt")
        ))

        search(NotQuery(RegexQuery(Field.Title, "foo/bar"))) must beEqualTo("1,2")
      }

      "truncate prefix searches that query for too many terms" in new BaseScope {
        override val maxExpansionsPerTerm = 4
        index.addDocuments(Vector.tabulate(5) { i => buildDocument(i.toLong).copy(text=s"duck duck goose${i}") })
    ...
    "#highlight" should {
      "return empty result on document it cannot match" in new BaseScope {
        ...
        index.highlight(1L, PhraseQuery(Field.Text, "dog")) must beEqualTo(Vector())
      }

      "match a word" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L).copy(text="i am cow hear me moo")))
        index.highlight(1L, PhraseQuery(Field.Text, "cow")) must beEqualTo(Vector(Utf16Highlight(5, 8)))
      }

      "match overlapping words" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L).copy(text="this is a hard, hard, hard query")))
        index.highlight(1L, PhraseQuery(Field.Text, "hard, hard")) must beEqualTo(Vector(Utf16Highlight(10,14), Utf16Highlight(16,20), Utf16Highlight(22,26)))
      }

      "return utf-16 indexes" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L).copy(text="\ud86d\udfa9 foo")))
        index.highlight(1L, PhraseQuery(Field.Text, "foo")) must beEqualTo(Vector(Utf16Highlight(3, 6)))
      }
    }
    ...
    "#highlights" should {
      "return empty result on document it cannot match" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L).copy(text="i am cow hear me moo")))
        index.highlights(Vector(1L), PhraseQuery(Field.Text, "cow")) must beEqualTo(Map(
          1L -> Vector(Utf16Snippet(0, 20, Vector(Utf16Highlight(5, 8))))
        ))
      }

      "return empty result when passed empty query" in new BaseScope {
        index.addDocuments(Vector(buildDocument(1L).copy(text="i am cow hear me moo")))
        index.highlights(Vector(), PhraseQuery(Field.Text, "cow")) must beEqualTo(Map())
      }
      ...
      "map documents to each other correctly" in new BaseScope {
        ...
        index.highlights(Vector(1L, 2L), PhraseQuery(Field.Text, "cow")) must beEqualTo(Map(
          1L -> Vector(Utf16Snippet(0, 20, Vector(Utf16Highlight(5, 8)))),
          2L -> Vector(Utf16Snippet(0, 28, Vector(Utf16Highlight(5, 8))))
        ))
      }
      ...
      "highlight prefixed matches" in new BaseScope {
        ...
        index.highlights(Vector(1L), PrefixQuery(Field.Text, "co")) must beEqualTo(Map(
          1L -> Vector(Utf16Snippet(0, 20, Vector(Utf16Highlight(5, 8))))
        ))
      }
      ...
      "highlight '_all' prefixed matches as 'text'" in new BaseScope {
        ...
        index.highlights(Vector(1L), PrefixQuery(Field.All, "co")) must beEqualTo(Map(
          1L -> Vector(Utf16Snippet(0, 20, Vector(Utf16Highlight(5, 8))))
        ))
      }
    }
    ...
    "#delete" should {
      ...
      "leave the index in a workable state" in new BaseScope {
        ...
        index.highlights(Vector(1L), query) must beEmpty
        index.addDocuments(Vector(buildDocument(1L)))
        search(query) must beEqualTo("1")
        index.highlight(1L, query) must not(beEmpty)
        index.highlights(Vector(1L), query) must not(beEmpty)
      }
    }
  }
}
