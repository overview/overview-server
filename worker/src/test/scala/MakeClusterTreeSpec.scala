...
class GenerateClustersSpec extends Specification {
  ...
  "ConnectedComponents" should {
    ...
    "find one component" in {
      ...
      component2.first must beEqualTo(1)  // component found should contain the node we started from
...
//      components.first.first must not beEqualTo(components.last.first)  // two components must have different nodes
...
/*  
 "TreeBuilder" should {
   "build a trivial tree" in {
     
   }
   
   "build a non-trivial tree" in {
