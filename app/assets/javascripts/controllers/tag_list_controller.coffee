...
tag_list_controller = (div, remote_tag_list, selection) ->
  view = new TagListView(div, remote_tag_list)

  view.observe('add-clicked', (tag) -> remote_tag_list.add_tag_to_selection(tag, selection))
  view.observe('remove-clicked', (tag) -> remote_tag_list.remove_tag_from_selection(tag, selection))
