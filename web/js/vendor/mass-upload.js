(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}g.MassUpload = f()}})(function(){var define,module,exports;return (function(){function r(e,n,t){function o(i,f){if(!n[i]){if(!e[i]){var c="function"==typeof require&&require;if(!f&&c)return c(i,!0);if(u)return u(i,!0);var a=new Error("Cannot find module '"+i+"'");throw a.code="MODULE_NOT_FOUND",a}var p=n[i]={exports:{}};e[i][0].call(p.exports,function(r){var n=e[i][1][r];return o(n||r)},p,p.exports,r,e,n,t)}return n[i].exports}for(var u="function"==typeof require&&require,i=0;i<t.length;i++)o(t[i]);return o}return r})()({1:[function(_dereq_,module,exports){
(function (global){
//     Backbone.js 1.4.0
...
},{"jquery":2,"underscore":3}],2:[function(_dereq_,module,exports){
/*!
...
//   retryAllUploads().
module.exports = MassUpload = class MassUpload extends Backbone.Model {
  defaults() {
    return {
      status: 'waiting',
  ...
  constructor(options) {
    super({}, options);
    this._removedUploads = []; // uploads removed by the user, still on the server
  }
  ...
  initialize(attributes, options) {
    ...
    resetUploadProgress = () => {
      return this.set({
        uploadProgress: this._uploadProgress.pick('loaded', 'total')
      });
    };
    ...
    this.listenTo(this.uploads, 'change', (upload) => {
      return this._onUploadChanged(upload);
    });
    this.listenTo(this.uploads, 'reset', () => {
      return this._onUploadsReset();
    });
    return this.prepare();
  }
  ...
  prepare() {
    ...
    this.uploader.callbacks = {
      ...
      onProgress: (upload, progressEvent) => {
  ...
  addFiles(files) {
    return this._uploadProgress.inBatch(() => {
  ...
  abort() {
    this.uploads.each((upload) => {
      return this.removeUpload(upload);
    });
    this.uploads.reset();
    return this.prepare();
  }
  ...
  _mergeUploadError(upload, prevError, curError) {
    ...
    newErrors = this.get('uploadErrors').slice(0); // shallow copy
    index = _.sortedIndex(newErrors, {
      upload: upload
    }, function(x) {
      return x.upload.id;
    });
    if (prevError == null) { // new error: insert curError
      newErrors.splice(index, 0, {
        upload: upload,
        error: curError
      });
    } else if (curError == null) { // old error: remove prevError
      newErrors.splice(index, 1); // replace prevError with curError
    } else {
  ...
  _tick() {
    ...
    if (upload != null) {
      if (upload.get('deleting')) {
        this.deleter.run(upload);
      } else {
        this.uploader.run(upload);
      }
    }
...
},{"./MassUpload/FileDeleter":5,"./MassUpload/FileLister":7,"./MassUpload/FileUploader":8,"./MassUpload/State":9,"./MassUpload/UploadCollection":11,"./MassUpload/UploadProgress":12,"backbone":1,"underscore":3}],5:[function(_dereq_,module,exports){
...
module.exports = FileDeleter = class FileDeleter {
  ...
  run(upload) {
    ...
    return this.doDeleteFile(upload, ((error) => {
      if (error) {
        return this._onError(upload, error);
      } else {
        return this._onSuccess(upload);
      }
    }));
  }
  ...
  _onSuccess(upload) {
    ...
    if (typeof (base = this.callbacks).onSuccess === "function") {
      base.onSuccess(upload);
    }
    return this._onStop(upload);
  }

  _onError(upload, error) {
    var base;
    if (typeof (base = this.callbacks).onError === "function") {
      base.onError(upload, error);
    }
    return this._onStop(upload);
  }
...
module.exports = FileInfo = class FileInfo {
...
FileInfo.fromJson = function(obj) {
  return new FileInfo(obj.name, obj.lastModified, obj.total, obj.loaded);
};

FileInfo.fromFile = function(obj) {
  return new FileInfo(obj.webkitRelativePath || obj.name, obj.lastModified, obj.size, 0);
};
...
module.exports = FileLister = class FileLister {
  ...
  run() {
    ...
    return this.doListFiles(((progressEvent) => {
      var base1;
      return typeof (base1 = this.callbacks).onProgress === "function" ? base1.onProgress(progressEvent) : void 0;
    }), ((error, fileInfos) => {
      if (error != null) {
        return this._onError(error);
      } else {
        return this._onSuccess(fileInfos);
      }
    }));
  }
...
module.exports = FileUploader = class FileUploader {
  ...
  run(upload) {
    ...
    return this._abortCallback = this.doUpload(upload, ((progressEvent) => {
      return this._onProgress(upload, progressEvent);
    }), ((error) => {
      if (error) {
        return this._onError(upload, error);
      } else {
  ...
  abort() {
    if (this._upload && !this._aborting) {
      this._aborting = true;
  ...
  _onProgress(upload, progressEvent) {
    var base;
    return typeof (base = this.callbacks).onProgress === "function" ? base.onProgress(upload, progressEvent) : void 0;
  }
  ...
  _onStop(upload) {
    ...
    this._abortCallback = null;
    this._aborting = false;
    return typeof (base = this.callbacks).onStop === "function" ? base.onStop(upload) : void 0;
  }

};


},{"./FileInfo":6}],9:[function(_dereq_,module,exports){
var State;
...
module.exports = Upload = (function() {
  class Upload {
    ...
    lastModified() {
      var ref;
      return this._lastModified != null ? this._lastModified : this._lastModified = (ref = this.file) != null ? ref.lastModified : void 0;
    }
    ...
    updateWithProgress(progressEvent) {
      ...
      fileInfo = new FileInfo(this.id, this.lastModified(), progressEvent.total, progressEvent.loaded);
      return this.set({
        fileInfo: fileInfo
      });
    }
    ...
    hasConflict() {
      return (this.fileInfo != null) && (this.file != null) && (this.fileInfo.name !== this.id || this.fileInfo.total !== this.size() || this.fileInfo.lastModified !== this.lastModified());
    }
...
},{"./FileInfo":6,"backbone":1,"underscore":3}],11:[function(_dereq_,module,exports){
var Backbone, Upload, UploadCollection, UploadPriorityQueue, _;
...
UploadPriorityQueue = class UploadPriorityQueue {
  constructor() {
    this._clear();
  }
  ...
  uploadAttributesToState(uploadAttributes) {
    var ret;
    ret = uploadAttributes.error != null ? null : uploadAttributes.deleting ? 'deleting' : uploadAttributes.uploading ? 'uploading' : (uploadAttributes.file != null) && (uploadAttributes.fileInfo != null) && uploadAttributes.fileInfo.loaded < uploadAttributes.fileInfo.total ? 'unfinished' : (uploadAttributes.file != null) && (uploadAttributes.fileInfo == null) ? 'unstarted' : null; // can't do anything
    return ret;
  }
...
module.exports = UploadCollection = (function() {
  class UploadCollection {
    ...
    remove(upload) {
      var index;
      index = this.models.indexOf(upload);
      if (index === -1) {
        throw 'Upload not found';
      }
      upload.off('all', this._onUploadEvent, this);
      delete this._idToModel[upload.id];
      this._priorityQueue.remove(upload);
      this.trigger('remove', upload, this);
      this.models.splice(index, 1);
...
},{"./Upload":10,"backbone":1,"underscore":3}],12:[function(_dereq_,module,exports){
...
// attributes.
module.exports = UploadProgress = (function() {
  class UploadProgress extends Backbone.Model {
    initialize() {
      var collection;
    ...
    change(model) {
      ...
      if (oldProgress != null) {
        ...
        return this._idToLastKnownProgress[model.id] = newProgress; // if there's no oldProgress, then an 'add' is coming
      }
    }
