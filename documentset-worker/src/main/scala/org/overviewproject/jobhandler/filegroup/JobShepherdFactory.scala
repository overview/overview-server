...
class FileGroupJobShepherdFactory extends JobShepherdFactory {
  ...
  override def createShepherd(documentSetId: Long, job: FileGroupJob, taskQueue: ActorRef, progressReporter: ActorRef): JobShepherd =
    job match {
      case CreateDocumentsJob(fileGroupId, options) =>
        CreateDocumentsJobShepherd(documentSetId, fileGroupId, options, taskQueue, progressReporter)
