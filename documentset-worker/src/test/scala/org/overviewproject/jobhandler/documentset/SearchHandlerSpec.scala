...
class SearchHandlerSpec extends Specification with Mockito {
  ...
  "SearchHandler" should {
    ...
    class SearchHandlerWithSearchFailure extends SearchHandler with SearchHandlerComponents {
      override val storage = mock[Storage]

      storage.searchExists(anyLong, anyString) returns false
      storage.createSearchResult(anyLong, anyString) returns 1l
      storage.queryForProject(anyLong, anyString) returns "search terms"
    ...
    trait SearchHandlerSetup extends Scope {
      ...
      def createSearchHandlerParent(searchExists: Boolean, parentProbe: ActorRef, documentSearcherProbe: ActorRef)(implicit system: ActorSystem): ActorRef =
